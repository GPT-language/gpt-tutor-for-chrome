...
export const createKeyMapper = (options: HyperStorageOptionsObj) => {
    const mapStateKeyToStorageKey = (key: string, mode: keyof HyperStorageOptionsObj = 'localStorage') => {
    ...
    const getStateKeyFromStorageKey = (key: string, mode: keyof HyperStorageOptionsObj = 'localStorage') => {
        const media = options[mode]
        if (media === false) return key
