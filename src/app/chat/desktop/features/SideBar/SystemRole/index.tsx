...
const SystemRole = memo(() => {
    ...
    const [showSystemRole, toggleSystemRole] = useGlobalStore((s) => [s.preference.showSystemRole, s.toggleSystemRole])
    ...
    return (
        <Flexbox height={'fit-content'}>
            <SidebarHeader
                actions={<ActionIcon icon={Edit} onClick={handleOpenWithEdit} size={'small'} title={t('edit')} />}
            ...
            >
                {!init ? (
                    <Skeleton active avatar={false} style={{ marginTop: 12, paddingInline: 16 }} title={false} />
                ) : (
                    <>
                        <EditableMessage
