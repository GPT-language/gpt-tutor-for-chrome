          ...
          fileWords.forEach((word) => {
            ...
            if (word.nextReview > current) {
              if (
                !closest ||
                (closest.nextReview &&
                  word.nextReview.getTime() - current.getTime() < closest.nextReview.getTime() - current.getTime())
              ) {
                closest = word
              }
            }
        ...
        {words.map((word, index) => (
          <div
            ...
            style={{
              ...
              cursor: 'pointer',
              backgroundColor: selectedWord && word.text === selectedWord.text ? rgb(255, 255, 0) : 'transparent',
            }}
  ...
  return (
    <SidebarContainer $showSidebar={showSidebar}>
      ...
      >
        ...
        />
        <input ref={fileInputRef} type='file' onChange={handleFileChange} accept='.csv' style={{ display: 'none' }} />
      </div>
      <ContentContainer>
        <div style={{ minHeight: '160px', width: '100%' }}>
          {isGridView ? (
            <GridView words={displayWords} selectedWord={selectedWord ?? words[0]} onWordClick={handleWordClick} />
          ) : (
