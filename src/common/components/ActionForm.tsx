...
export function ActionForm(props: IActionFormProps) {
  ...
  const commandPromptCaption = (
    <div className={styles.promptCaptionContainer}>
      ...
      <div>{t('Command prompt example: Please translate the following text from ${sourceLang} to ${targetLang}.')}</div>
      <div>{t('Placeholders')}:</div>
      ...
      >
        ...
        >
          <span>
            ...
            <Button isLoading={loading} size='compact' disabled={props.action?.mode === 'built-in' && !isAdmin}>
              {t('Save')}
            </Button>
          </span>
        </StatefulTooltip>
