  ...
  useLayoutEffect(() => {
    const handleResize = () => {
      ...
      let count = Math.floor(
        (headerWidth - paddingWidth - languagesSelectorWidth - 102 - iconWithTextWidth * (hasActivateAction ? 1 : 0)) /
          (iconGap + iconWidth)
      )
        ...
        await translate(
          {
            ...
            onMessage: async (message) => {
              ...
              setTranslatedText((translatedText) => {
                ...
                const newTranslatedText = message.isFullText ? message.content : translatedText + message.content
            ...
            onFinish: (reason) => {
              ...
              setTranslatedText((translatedText) => {
                ...
                const { messageId, conversationId, activateAction, updateTranslationText } = useChatStore.getState()
                      ...
                      <>
                        <Tooltip content={t('Speak')} placement='bottom'>
                          <div className={styles.actionButton} onClick={handleEditSpeakAction}>
                            {isSpeakingEditableText ? <SpeakerMotion /> : <RxSpeakerLoud size={15} />}
                          </div>
                        </Tooltip>
                    ...
                    {errorMessage ? (
                      ...
                      >
                        <div ref={translatedContentRef} className={styles.popupCardTranslatedContentContainer}>
                          <div>
                            {showTextParser ? (
                              <TextParser jsonContent={jsonText} setOriginalText={handTextParserClick}></TextParser>
                            ) : null}
                            <>
                                ...
                                ([actionName, { text, format, messageId, conversationId }]) => (
                                  <div key={actionName}>
                                    <Accordion
                                      ...
                                      onChange={({ expanded }) => handleAccordionChange(expanded)}
                                      overrides={{
                                        ToggleIcon: {
                                          component: () =>
                                            actionName === activeKey ? (
                                              ...
                                              <mdIcons.MdArrowDropDown size={24} />
                                            ),
                                        },
                                      }}
                                      accordion={true}
                                          ...
                                          <>
                                            ...
                                            <Tooltip content={t('Speak')} placement='bottom'>
                                              <div
                                                className={styles.actionButton}
                                                onClick={() =>
                                                  handleTranslatedSpeakAction(messageId, conversationId, text)
                                                }
                                              >
                                            ...
                                            <Tooltip content={t('Copy to clipboard')} placement='bottom'>
                                              <div className={styles.actionButton}>
                                                <CopyButton text={text} styles={styles}></CopyButton>
                                              </div>
                                            </Tooltip>
                                            <Tooltip content={t('Add to Anki')} placement='bottom'>
                                              <div
                                                onClick={() =>
                                                  addToAnki(
                                                    selectedGroup + ':' + actionName.split(':')[0], // assuming key is activateAction.name:editableText
                                                    editableText,
                                                    text
                                                  )
                                                }
